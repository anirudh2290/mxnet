...
if __name__ == '__main__':
    ...
    time_cost_epoch = None
    for epoch in range(num_epoch):
        start_time_epoch = time.time()
        logging.info("Epoch starting: {}".format(epoch))
        nbatch = 0
        end_of_batch = False
        data_iter.reset()
        metric.reset()
        ...
        if kv is not None:
            logging.info("Before row_sparse pull for epoch: {}".format(epoch))
            ...
            logging.info("------------------------------------")
            logging.info("------------------------------------")
            logging.info(next_batch.data[0].shape[1])
            logging.info(next_batch.data[0].shape[0])
            logging.info("After row_sparse pull for epoch: {}".format(epoch))
        while not end_of_batch:
            ...
            try:
                ...
                if kv is not None:
                    ...
                    row_sparse_pull(kv, 'w', next_batch.data[0], mod._exec_group.slices, weight_array, -index)
                    logging.info("After row_sparse pull for epoch: {}".format(epoch))
                    logging.info("nbatch: {}".format(nbatch))
            except StopIteration:
                end_of_batch = True
            # accumulate prediction accuracy
            logging.info("nbatch: {}".format(nbatch))
            mod.update_metric(metric, batch.label)
            print "batch complete: num_batches: ", nbatch
        ...
        logging.info('cpu/32 cores| {} | {} | {} | {}'.format(str(num_worker), batch_size, str(args.output_dim), str(time_cost_epoch)))
    if profiler:
        mx.profiler.profiler_set_state('stop')
    end = time.time()
    time_cost = end - start
    logging.info('num_worker = ' + str(num_worker) + ', time cost = ' + str(time_cost))
    logging.info('cpu/32 cores| {} | {} | {} | {}'.format(str(num_worker), batch_size, str(args.output_dim), str(time_cost)))

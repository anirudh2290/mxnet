...
import numpy as np
import mxnet as mx
from mxnet.base import c_handle_array, c_str_array
...
class Predictor2(object):
    ...
    def __init__(self, symbol_file,
        ...
        if not isinstance(input_ndarrays, list):
            raise ValueError("inputs ndarray must be a list")
        input_list = c_handle_array(input_ndarrays)
        provided_input_dtype_names = []
        provided_input_dtype_types = []
        for k, v in provided_input_dtype_dict.items():
            provided_input_dtype_names.append(k)
            provided_input_dtype_types.append(v)
        print(provided_input_dtype_names)
        print(provided_input_dtype_types)

        provided_input_dtype_types = c_array(ctypes.c_int, array('i', provided_input_dtype_types))
        provided_input_dtype_names = c_str_array(provided_input_dtype_names)
        ...
        _check_call(_LIB.MXPredCreateEx(
            ...
            mx_uint(len(provided_input_dtype_dict)),
            provided_input_dtype_names,
            provided_input_dtype_types,
            ctypes.byref(handle)))
        self.handle = handle
    ...
    def forward(self, **kwargs):
        ...
        **kwargs
        ...
        for k, v in kwargs.items():
            ...
            v = np.ascontiguousarray(v, v.dtype)
...
def test_predictor():
    ...
    out = data * 2
    out3 = out * data2
    x = Predictor2(out3.tojson(), {"data": (1, 1), "data2": (1, 1)}, [mx.nd.array([[1]], dtype=np.float16), mx.nd.array([[2]], dtype=np.float16)], {"data": 2, "data2": 2})
    x.forward(data=mx.nd.array([[1]], dtype=np.float16).asnumpy())
    out = x.get_output(0)
    del x
